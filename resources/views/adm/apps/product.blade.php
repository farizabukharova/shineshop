...
@section('content')
    <form class="form-check mt-3" action="{{route('adm.apps.search')}}" method="GET">
    ...
    <a class="btn btn-outline-success mt-5" href="{{route('adm.apps.create')}}">{{ __('messages.adds') }}</a>
    <table class="table">
        ...
        @for($i=0; $i<count($products); $i++)
            <tr>
                ...
                <td>
                  <a href="{{route('adm.apps.edit', $products[$i]->id)}}" class="btn btn-success">{{ __('messages.Edit') }}</a>
                </td>
                <td>
                    <form action="{{route('adm.apps.destroy', $products[$i]->id)}}" method="post">
                        @csrf
                        @method('DELETE')
